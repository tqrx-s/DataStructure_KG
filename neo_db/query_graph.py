...
def query_branch(name, deep):
    if deep == '二度查询':
        graph.run(
            ...
            # '"file:///D:/Program Files (x86)/Neo4j/Neo4j Desktop/DSsource/%s.json", {writeNodeProperties:true})' % (name,name))
            '"file:///D:/%s.json", {writeNodeProperties:true})' % (name,name))
        ...
        graph.run(
            ...
            # '"file:///D:/Program Files (x86)/Neo4j/Neo4j Desktop/DSsource/%s.json", {writeNodeProperties:true})' % (name,name))
            '"file:///D:/Program Files(x86)\%s.json", {writeNodeProperties:true})' % (name,name))
            # '"knows-with-node-properties.json", {writeNodeProperties:true})' % (name))
    elif deep == '四度查询':
        graph.run(
    ...
    with open(
            r"D:/%s.json" % name,
            # r"D:\\Program Files (x86)\\Neo4j\Neo4j Desktop\\DSsource\\%s.json" % name,
