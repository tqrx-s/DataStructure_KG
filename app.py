...
# from recommend.codebase.SASRec.recommend import recommend_with_course, recommend_new_user
import numpy as np
import pandas as pd
import csv
import os
...
def login_action():
    ...
    if account in users and users[account] == password:
        ...
        print("fail")
        return jsonify(status="error", message="!0")
...
# def recommend():
#     train_data_list = get_train_data_list('/recommend/data2.csv')
#     return render_template('knowledge/recommend.html')

# @app.route('/make_hobby.html', methods=['GET', 'POST'])
# def make_hobby():
#     train_data_list = get_train_data_list('/recommend/data2.csv')
#     return render_template('knowledge/make_hobby.html')

# latest_course_indexes = []
# # 直接推荐
# @app.route('/recommend_Id', methods=['GET', 'POST'])
# def recommend_Id():
#     if not latest_course_indexes:
#         userId = 0
#         ans = recommend_with_course(userId)
#     else:
#         ans = recommend_new_user(latest_course_indexes)
#     courses = [{'name': course} for course in ans]
#     # print(courses)
#     print("late1=",latest_course_indexes)
#     latest_course_indexes.clear()
#     print("late2=",latest_course_indexes)
#     return jsonify(courses), 200

# # 获取课程名单
# @app.route('/recommend_options', methods=['GET', 'POST'])
# def recommend_options():
#     name = request.args.get('search_kw')
#     data = []
#     # 读取 CSV 文件
#     with open('raw_data/options.csv', mode='r', encoding='utf-8') as file:
...
# @app.route('/ChooseById', methods=['POST'])
# def ChooseById():
#     course_index = request.json.get("id")
#     course_index = int(course_index)
#     latest_course_indexes.insert(0, course_index)
#     if len(latest_course_indexes) > 10:
#         del latest_course_indexes[10:]
#     return jsonify({'message': '课程选择成功', 'course_index': course_index}), 200

# 诊断界面
@app.route('/diagnose.html', methods=['GET', 'POST'])
def diagnose():
    return render_template('diagnose.html')

@app.route('/diagnose2.html', methods=['GET', 'POST'])
def diagnose2():
    return render_template('diagnose2.html')

# 知识点检索
@app.route('/d_app_submit', methods=['GET', 'POST'])
def d_app_submit():
    print("app!name:",name)
    name = request.args.get('name')
    json_data = query(str(name))
